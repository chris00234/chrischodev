...
const Hero = () => {
  ...
  const roles = ['Full-Stack Developer', '.NET Specialist', 'Database Architect', 'System Designer']
  ...
  return (
    <section className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-accent-500/10 rounded-full blur-3xl animate-pulse-slow" style={{animationDelay: '1s'}}></div>
      </div>
          ...
          <div className="flex justify-center space-x-8 mb-16">
            {techIcons.map(({ icon: Icon, label }) => (
              <div key={label} className="flex flex-col items-center group cursor-pointer">
                ...
                <span className="text-slate-400 text-sm mt-2 group-hover:text-white transition-colors">
                  {label}
                </span>
              </div>
            ))}
